...
#     asyncio.run(Move.random_async_move())


class Move:
    def __init__(self, events):
        self.events = events
        self.flap_thread = None
    ...
    def move(self):
        ...
        while not self.events.shutdown_event.is_set():
            ...
            if self.events.face_detected_event.is_set() and (
                self.flap_thread is None or not self.flap_thread.is_alive()
            ):
                ...
                # self.flap_wings_face_detection()
                # time.sleep(TIME_TO_TRACK_FACE)
                self.flap_thread = threading.Thread(
                    target=self.flap_wings_face_detection, daemon=True
                )
                ...
                #     interval=8.0,
                #     function=self.resume_tracking_faces,
                # )
                # timer.start()

            # if (
            #     not self.events.resume_face_tracking_event.is_set()
            #     and not self.events.head_pat_event.is_set()
