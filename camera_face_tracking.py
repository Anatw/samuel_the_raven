...
FRAME_WIDTH = (
    1920  # The rightes point in the image. Constant value reteieved with frams.shape
)
FRAME_HEIGHT = 1080  # The lowest (buttom) point in the image. Constant value reteieved with frams.shape. The image top point value is 0.
cv_scaler = 6  # (4) this has to be a whole number
TIME_TO_TRACK_FACE = (
    100  # The time in seconds to track the face before going back to random movements.
)
BOREDOM_TIME = 60  # The time in seconds to stay in idle movement mode even when a face is detected.
IMAGE_THIRD = FRAME_HEIGHT / 3
UNRECOGNIZED_FACE = "unknown"
FACE_METCH_TOLERANCE = 0.45  # lower = stricter
...
class FaceDetecion:
    def __init__(
        ...
        self.currently_tracked_face = (
            UNRECOGNIZED_FACE  # live - who’s on screen right now
        )
        self.tracked_face = (
            UNRECOGNIZED_FACE  # snapshot at the start of a tracking session
        )
    ...
    def search_for_faces(self, frame, return_boxes: bool = False):
        ...
        for face_encoding in self.face_encodings:
            ...
            # See if the face has a match in the known faces pool
        ...
        if (
            name == self.bored_from_face  # same person we got bored of
            and time.time() < self.bored_until  # boredom window still open
        ):
        ...
        self.bored_from_face = UNRECOGNIZED_FACE  # clear boredom markers
        self.bored_until = 0

        if self.samuel.events.resume_face_tracking_event.is_set():
            self._begin_tracking(name)

        if return_boxes:  # if caller only wants names and raw boxes back:
    ...
    def _tracking_timeout(self):
        ...
        if (
            self.tracked_face == self.currently_tracked_face  # same human stayed
            and self.tracked_face != UNRECOGNIZED_FACE  # ignore 'unknown'
        ):
    ...
    def samuel_track_face(self):
        if self.samuel.events.face_detected_event.is_set():
            # Add a Kraa random sound here! + random wing flap and mouth move every once in a while.
            for top, right, bottom, left in self.face_locations:
    ...
    def face_detection_and_tracking(self):
        ...
        try:
            while not self.samuel.events.shutdown_event.is_set():
                ...
                self.search_for_faces(processed)
